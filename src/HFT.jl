...
using FinancialStruct:cOrderReq
using FinancialStruct:cCancelReq     
using FinancialStruct:cCancelDetail
using FinancialStruct:cOrderRsp    
using FinancialStruct:cOrder       
using FinancialStruct:cTrade       
using FinancialStruct:cPosition    
...
function __init__()
    # 确保 artifact 可用
    lib_dir = artifact"hftbt_lib"
    # 根据平台设置库路径
    global dlfile
    if Sys.iswindows()
        dlfile = joinpath(lib_dir, "hftbt.dll")
    elseif Sys.islinux()
#        dlfile = joinpath(lib_dir, "libmy_dolphindb_api.so")
    end
    # 验证库文件是否存在
    if !isfile(dlfile)
        @error "hftbt library files not found. Please make sure the package is installed correctly."
    end
...
##############################################md_api################################################################
#/************************ 获取历史行情相关接口 begin ***************************/
#push!(LOAD_PATH, "D:/workspace/backtest/win64/struct_julia/")

#using marketdata_def
...
function hfttb_init(output_interval::Integer, create_options::Integer, price_dist_levels::Integer)
    sym = Libc.Libdl.dlsym(lib_tick, :hfttb_init)   # 获得用于调用函数的符号
...
function hfttb_set_cb_callback(on_tick_cb::Function, user_data::Ptr{Cvoid}=C_NULL)::Int
    ...
    sym = Libc.Libdl.dlsym(lib_tick, :hfttb_set_cb)
 ...
 *  解除撮合库
"""
function hfttb_release()::Int
    sym = Libc.Libdl.dlsym(lib_tick, :hfttb_release)
    ccall(sym, Cvoid, ())
    return Int(0)
end
export hfttb_release

"""
    hfttb_update(date::Integer)::Int
 *  更新撮合日期，初始化和切换日期时需要调用此函数。
 *
 * @param date      日期
"""
function hfttb_update(date::Integer)::Int
    sym = Libc.Libdl.dlsym(lib_tick, :hfttb_update)
...
function hfttb_push_securitytick(data::Cptr{cSecurityTickData})::Int
    sym = Libc.Libdl.dlsym(lib_tick, :hfttb_push_securitytick)
...
function hfttb_push_tbt(data::Cptr{cTickByTickData})::Int
    sym = Libc.Libdl.dlsym(lib_tick, :hfttb_push_tbt)
...
#    sym = Libc.Libdl.dlsym(lib_tick, :hfttb_get_tick)
...
function hfttb_refresh_tick(symbol::String)::Int
    sym = Libc.Libdl.dlsym(lib_tick, :hfttb_refresh_tick)
...
"""
    td_cancel(account_id::String, account_type::Integer, cancel_reqs::Vector{cCancelReq})::Vector{cCancelDetail}
 * 批量撤单，通过委托信息撤销单个和多个订单，同步异步使用一个接口
 *
 * @param account_id    资金账户id
 * @param account_type  资金账户类型，见AccountType定义
 * @param cancel_reqs   待撤单委托列表
 * @param req_num       待撤单委托列表长度
 * @param is_async      true：异步撤单； false: 同步撤单
 *
 * @return cancel_list  返回撤单详情列表(异步撤单时返回空)
"""
function td_cancel(account_id::String, account_type::Integer, cancel_reqs::Vector{cCancelReq}, is_async::Bool=true)::Vector{cCancelDetail}
    len = length(cancel_reqs)
    cancel_list = cCancelDetail[]
    sym = Libc.Libdl.dlsym(lib, :td_cancel)
    if is_async
        err = ccall(sym, Int32, (Ptr{UInt8}, Cint, Cptr{cCancelReq}, Cint, Cptr{Cptr{cCancelDetail}}, Ptr{Cint}), account_id, account_type, cancel_reqs, len, C_NULL, C_NULL)
    else
        len_r = Ref{Cint}(0)
        cancel_list_r = Ref{Cptr{cCancelDetail}}(C_NULL)
        err = ccall(sym, Int32, (Ptr{UInt8}, Cint, Cptr{cCancelReq}, Cint, Cptr{Cptr{cCancelDetail}}, Ptr{Cint}), account_id, account_type, cancel_reqs, len, cancel_list_r, len_r)
        if err == 0
            for i = 1:len_r.x
                cancel = unsafe_load(cancel_list_r[] + i - 1)
                push!(cancel_list, cancel)
...
"""
    td_get_order(order_id::String)::cOrder
 * 查订单详情
 *
...
"""
function td_get_order(order_id::String)::cOrder
...
"""
    td_get_indicator(date::String="")::cIndicator
 * 查策略实例的收益、风险等指标
 *
...
"""
function td_get_indicator(date::String="")::cIndicator
    if length(date) == 0
        date = C_NULL
    end
 ...
 * 设置成交回报事件回调函数
 *
 * @param on_trade      回调处理函数on_trade(trade::Ptr{cTrade}, user_data::Cptr{Cvoid})
 * @param user_data     用户自定义参数，与回调相关的任意类型数据，作为回调函数参数输入
"""
 ...
 * 设置订单委托应答事件回调函数
 *
 * @param on_order_rsp      回调处理函数on_order_rsp(order_rsp::Ptr{cOrderRsp}, user_data::Cptr{Cvoid})
 * @param user_data         用户自定义参数，与回调相关的任意类型数据，作为回调函数参数输入
"""
 ...
 * 设置撤单应答事件回调函数
 *
 * @param on_cancel_order   回调处理函数on_cancel_order(cancel_detail::Cptr{cCancelDetail}, user_data::Cptr{Cvoid})
 * @param user_data         用户自定义参数，与回调相关的任意类型数据，作为回调函数参数输入
"""
 ...
 * 设置订单状态变化事件回调函数
 *
 * @param on_order      回调处理函数on_order(order::Cptr{cOrder}, user_data::Cptr{Cvoid})
 * @param user_data     用户自定义参数，与回调相关的任意类型数据，作为回调函数参数输入
"""
